...
struct State {
    ...
    #[allow(dead_code)] // Why is this here twice?
...
impl State {
    async fn new(window: Window) -> Self {
        ...
        // one will result in all the colors coming out darker. If you want to support
        ...
        let config = wgpu::SurfaceConfiguration {
            ...
            view_formats: vec![], // This is a hint to the implementation to use the same format as the texture
        };
        ...
        let texture_bind_group_layout = 
            ...
            let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                ...
                primitive: wgpu::PrimitiveState {
                    topology: wgpu::PrimitiveTopology::TriangeList,
                    strip_index_format: None,
                    front_face: wgpu::FrontFace::Ccw,
                    cull_mode: Some(wgpu::Face::Back),
                    // Setting this to anything other than Fill requires
                    // Features::POLYGON_MODE_LINE or Features::POLYGON_MODE_POINT
...
async fn run() {
    ...
    let window = WindowBuilder::nnew().build(&event_loop).unwrap();
    ...
    event_loop.run(move |event, _, control_flow| {
        match event {
            ...
            Event::RedrawRequested(window_id) if window_id == state.window().id() => {
                ...
                match state.render() {
                    Ok(_) => {}
                    // Reconfigure the surface if it's lost or outdates
